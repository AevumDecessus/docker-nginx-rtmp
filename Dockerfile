...
# create directories
RUN mkdir -p /src && mkdir -p /config && mkdir -p /logs && mkdir -p /data && mkdir -p /recordings 

# Run updates
RUN apt-get update && apt-get upgrade -y && apt-get clean
RUN apt-get install -y build-essential wget libpcre3-dev zlib1g-dev libssl-dev
